...
@section('title', 'Tambah Surat Baru - LPM Ibnu Rusyd')
@section('content')
    <div class="bg-white rounded-lg shadow overflow-hidden">
        <div class="p-6 border-b border-gray-200">
            <h2 class="text-lg font-medium text-gray-900">Tambah Surat Baru</h2>
            <p class="mt-1 text-sm text-gray-600">Isi form di bawah untuk menambahkan surat baru ke arsip.</p>
        </div>

        <form action="{{ route('arsip_surat.store') }}" method="POST" enctype="multipart/form-data" class="p-6">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="deskripsi" id="deskripsi" rows="4"
                        class="w-full px-3 py-2 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-primary focus:border-primary">{{ old('deskripsi') }}</textarea>
                    @error('deskripsi')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
